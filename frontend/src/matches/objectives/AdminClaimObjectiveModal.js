...
const AdminClaimObjectiveModal = ({match, open, publicObjectives, secretObjectives, onConfirm, onCancel}) => {
    ...
    const onProceed = () => {
        axios.post('/api/admin/'+selectedPlayer+'/matches/'+match.matchId+'/objectives/'+selectedObjective)
            .then(onConfirm)
            .catch((error) => {
    ...
    return (
        <Modal
            ...
            open={showModal}
        >
            <Modal.Header>Admin claim objective</Modal.Header>
            <Modal.Content>
                <Modal.Description>
            ...
            <Modal.Actions>
                ...
                <Button onClick={onProceed} primary disabled={!selectedObjective}>Claim objective</Button>
            </Modal.Actions>
        </Modal>
    );
};
...
export default AdminClaimObjectiveModal;